...
void connectServos() {
  //Front Right
  //0=2500
  Servos[0].attach(7);//Hip
  Servos[1].attach(6);//Thigh
  Servos[2].attach(5);//Leg

  //Front Left
  //0=550
  Servos[3].attach(8);//Hip
  Servos[4].attach(9);//Thigh
  Servos[5].attach(10);//Leg

  //Back Right
  //0=2500
  Servos[6].attach(2);//Hip
  Servos[7].attach(3);//Thigh
  Servos[8].attach(4);//Leg

  //Back Left Leg
  Servos[9].attach(13);//Hip
  Servos[10].attach(12);//Thigh
  Servos[11].attach(11);//Leg
}
...
int angleToPulse(double angle, int i) {
  ...
  if (i == 0) {
    //Hip
    pulse = int(10.822 * angle) + 2350; //initial position
  }
  else if (i == 1) {
    //Thigh
...
//for debuging, moving this can destabilize the robot
void keyboardInput() {
...
void loop() {
  ...
  increment = 0.0001;//speed up the loop.
  for (double i = 0 ; i <= 0.99 ; i = i + increment) {//BR leg up
  ...
  for (double i = 0 ; i <= 0.99 ; i = i + increment) {//FR leg up
    unsigned long currentMillis = millis();
    if (currentMillis - previousMillis >= interval) {
      // save the last time you blinked the LED
      previousMillis = currentMillis;
  ...
  for (double i = 0 ; i <= 0.99 ; i = i + increment) {//BL leg up
    unsigned long currentMillis = millis();
    if (currentMillis - previousMillis >= interval) {
      // save the last time you blinked the LED
      previousMillis = currentMillis;
  ...
  for (double i = 0 ; i <= 0.99 ; i = i + increment) {//FL leg up
    unsigned long currentMillis = millis();
    if (currentMillis - previousMillis >= interval) {
      // save the last time you blinked the LED
      previousMillis = currentMillis;
